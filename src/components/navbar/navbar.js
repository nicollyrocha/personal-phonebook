...
function NavBar({ setLogado, dados }) {
	...
	function onClickPage(page) {
		...
		} else if (page.target.textContent === 'Logout') {
			history('/');
			localStorage.setItem('token', '');
			setLogado(false);
		} else {
	...
	return (
		<AppBar style={{ backgroundColor: '#eccccc', color: '#CF2E2E' }}>
					...
					<Box sx={{ flexGrow: 0 }}>
						<Tooltip title="Open settings">
							<IconButton
								...
								sx={{ p: 0 }}
							>
								<Avatar>{localStorage.getItem('user')[0].toUpperCase()}</Avatar>
							</IconButton>
						</Tooltip>
						<Menu
							sx={{ mt: '45px' }}
